...
Response GLMProvider::Complete(const Request& req) {
    Response out;
    if (cfg_.apiKey.empty()) {
        out.error = "GLM API key not set. Open Settings dialog and configure 'glm' provider.";
        return out;
    }
    ...
    if (out.content.empty())
        out.error = "Empty content in GLM response";
    return out;
}
